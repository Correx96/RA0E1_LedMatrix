...
/*
 * Code snippet by @siddharthlh24 for STM32 platform
...
unsigned char Rotary_process(ioport_ctrl_t *const  pinD1_ctrl, bsp_io_port_pin_t pinD1_number, ioport_ctrl_t *const  pinD2_ctrl, bsp_io_port_pin_t pinD2_number) {
  ...
  unsigned char pinstate = (digitalRead(pinD2_ctrl, pinD2_number) << 1) | digitalRead(pinD1_ctrl, pinD1_number);
...
{
    bsp_io_level_t temp;
    R_IOPORT_PinRead(p_ctrl, pin_number, &temp);
    return (temp == BSP_IO_LEVEL_LOW) ? 0 : 1;
}
